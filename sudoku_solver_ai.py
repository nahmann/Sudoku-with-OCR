...
class SudokuOCRSolver:
    ...
    def _load_model(self):
        ...
        try:
            self.keras_model_path = tf.keras.models.load_model(self.keras_model_path)
            print(f"Successfully loaded model from {self.keras_model_path}")
        except Exception as e:
            print(f"Error loading model: {e}")
            raise

    def _load_keras_model(self, model_path: str = 'mnist_digit_model.keras'):
        """Load the pretrained Keras model. Add this to your existing class."""
        try:
...
#### OCR METHODS
    ...
    def _process_cell(self, cell: np.ndarray, debug: bool) -> Optional[int]:
        ...
        print(f"Debug: Model object exists: {hasattr(self, 'model')}")
        print(f"Debug: Keras_model object exists: {hasattr(self, 'keras_model')}")
        if hasattr(self, 'model'):
            print(f"Debug: Model is None: {self.model is None}")
        if hasattr(self, 'keras_model'):
            print(f"Debug: Keras_model is None: {self.keras_model is None}")
        ...
        if debug:
            print(f"  Keras result: digit={predicted_digit}, confidence={confidence:.1f}%")
        ...
        # If confidence is low, you might want to apply additional validation
        # or return None for uncertain predictions
        if debug:
